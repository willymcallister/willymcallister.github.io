...
The last half of this article is an advanced look at some of the way things go wrong with a voltage divider. You can skip these sections if this is the first time you've studied the voltage divider. When you eventually come back and read through the whole article, you will know pretty much everything there is to know about the voltage divider.

The exercises include links to Circuit Sandbox, an interactive simulator.
...
A "voltage divider" is this pattern of resistors, with an input voltage applied to both ends, and an output voltage measured across one of the resistors.
...
It is a simple circuit with two resistors in series. The thing that tips you off that this is a voltage divider is one voltage connected to the ends, and another voltage measured across one of the resistors. We think of the voltage on the ends as the input voltage, and the voltage across the single resistor is the output. The little circles represent the *ports* of the voltage divider, and they are connected to something we can't see right now.

Our goal is to come up with an expression that relates output $v_{out}$ to input $v_{in}$. This is a very simple series circuit, so it won't take a lot of effort to figure out. 

Before we start, we make this important assumption,

>**Assumption:** Assume the current flowing out of the divider from its center node is zero. 
...
A good place to start is to find the current through $\text{R1}$ and $\text{R2}$. With the assumption, we know $\text{R1}$ and $\text{R2}$ have the same current, and we can consider them to be in series. 

$i_{\text{1}} = i_{\text{2}}\qquad$ and for now let's just call this $i$.

To find the current, we apply Ohm's Law, $v = i\,\text{R}$, and what we know about resistors in series: resistors in series add.
...
This give us current $i$ in terms of $v_{in}$ and both resistors. 

Next, we write an expression for $v_{out}$ using Ohm's Law,

$v_{out} = i \, \text{R2}$

We substitute for $i$ from the previous equation to get,

$v_{out} = \left (v_{in}\,\dfrac{1}{\text{R1} + \text{R2}} \right ) \text{R2}$

 and we have derived the voltage divider equation:

![Voltage divider equation]({{ site.baseurl }}{% link i/voltage-divider-equation.svg %}){: .centered }
...
The ratio of resistors is always less than $1$ for any values of $\text{R1}$ and $\text{R2}$. (Convince yourself of this.) If the resistor ratio is less than $1$, that means $v_{out}$ is always less than $v_{in}$. Input voltage $v_{in}$ is scaled down to $v_{out}$  by a fixed ratio determined by the resistor values. This is where the circuit gets its nickname: *voltage divider*.

### Example - the voltage divider equation

Let's do a specific example. Find $v_{out}$ for this schematic,

$v_{out}  = v_{in}\,\dfrac{\text{R2}}{\text{R1} + \text{R2}}$ 

![](https://ka-perseus-images.s3.amazonaws.com/5d860df1a6275a4013ffebb791ca2c003e77b53f.svg){: .centered }

We insert the input voltage and resistor values into the voltage divider equation, remembering the *bottom* resistor, $\text{R2}$, goes in the numerator. 
...
>[Circuit sandbox model](http://spinningnumbers.org/circuit-sandbox/index.html?value=[["r",[200,40,0],{"name":"R1","r":"1k","_json_":0},["2","1"]],["v",[112,80,0],{"name":"vin","value":"dc(12)","_json_":1},["2","0"]],["r",[200,120,0],{"name":"R2","r":"3k","_json_":2},["1","0"]],["w",[112,40,200,40]],["w",[112,168,112,128]],["w",[112,40,112,80]],["g",[160,168,0],{"_json_":6},["0"]],["w",[112,168,160,168]],["w",[200,168,160,168]],["w",[200,88,200,104]],["w",[200,120,200,104]],["w",[200,104,232,104]],["w",[232,104,232,104]],["view",0,0,2,"50","10","1G",null,"100","0.01","1000"]]) of the example voltage divider. Click on DC in the top menu to find the DC operating point.
...
>[Circuit sandbox model](http://spinningnumbers.org/circuit-sandbox/index.html?value=[["r",[200,40,0],{"name":"R1","r":"90k","_json_":0},["2","1"]],["v",[112,80,0],{"name":"vin","value":"dc()","_json_":1},["2","0"]],["r",[200,120,0],{"name":"R2","r":"10k","_json_":2},["1","0"]],["w",[112,40,200,40]],["w",[112,168,112,128]],["w",[112,40,112,80]],["g",[160,168,0],{"_json_":6},["0"]],["w",[112,168,160,168]],["w",[200,168,160,168]],["w",[200,88,200,104]],["w",[200,120,200,104]],["w",[200,104,232,104]],["w",[232,104,232,104]],["view",0,0,2,"50","10","1G",null,"100","0.01","1000"]]) of Problem 3. Add a voltage value to $v_{in}$ to get the output voltage you want.
...
>[Circuit sandbox model](http://spinningnumbers.org/circuit-sandbox/index.html?value=[["r",[200,40,0],{"name":"R1","r":"30k","_json_":0},["2","1"]],["v",[112,80,0],{"name":"vin","value":"dc(5)","_json_":1},["2","0"]],["r",[200,120,0],{"name":"R2","r":"","_json_":2},["1","0"]],["w",[112,40,200,40]],["w",[112,168,112,128]],["w",[112,40,112,80]],["g",[160,168,0],{"_json_":6},["0"]],["w",[112,168,160,168]],["w",[200,168,160,168]],["w",[200,88,200,104]],["w",[200,120,200,104]],["w",[200,104,232,104]],["w",[232,104,232,104]],["view",0,0,2,"50","10","1G",null,"100","0.01","1000"]]) of Problem 3. Pick a value for $\text R2$ to get the desired output voltage.
...
>[Circuit sandbox model](http://spinningnumbers.org/circuit-sandbox/index.html?value=[["r",[200,40,0],{"name":"R1","r":"","_json_":0},["1","3"]],["v",[112,80,0],{"name":"vin","value":"dc(1)","_json_":1},["2","0"]],["r",[200,120,0],{"name":"R2","r":"","_json_":2},["3","0"]],["w",[112,168,112,128]],["w",[112,40,112,80]],["g",[160,168,0],{"_json_":5},["0"]],["w",[112,168,160,168]],["w",[200,168,160,168]],["w",[200,88,200,104]],["w",[200,120,200,104]],["w",[200,104,232,104]],["w",[232,104,232,104]],["a",[152,40,0],{"color":"magenta","offset":"0","_json_":12},["2","1"]],["w",[112,40,152,40]],["w",[200,40,168,40]],["view",0,0,2,"50","10","1G",null,"100","0.01","1000"]]) of Problem 4. Enter both resistor values to achieve the design specifications: $v_{out} = v_{in}/2$ and $p = 10 \,\mu\text{W}$.

## Review the current assumption (optional) 

A voltage divider doesn't do anything useful unless its output is connected to something. You should be aware of what happens when a voltage divider is connected to a load.

Remember way back at the start we made an assumption? We assumed the current flowing away from the node between the two resistors was $0$. That let us treat $\text{R1}$ and $\text{R2}$ as if they were in series, and we developed the voltage divider equation. Let's check what happens if the assumption is not true. 

### Operating the voltage divider near mid-range

To start this discussion, we set up a voltage divider with $\text{R1} = \text{R2}$. With matched resistors, the expected $v_{out}$ of the voltage divider is half the input voltage, $v_{out} = 0.5\,v_{in}$. If we connect a load resistor $\text R_{\text L}$ in parallel with $\text R2$, this will cause some current, $i_\text L$, to flow away from the center node through $R_\text L$. 

![](https://ka-perseus-images.s3.amazonaws.com/8305a10000c6f4066b2a4a46d353b1a5c0cc2ef6.svg){: .centered }

Here's the question: Does the voltage divider still work, or does our voltage divider story collapse? 

Resistor $\text R_\text L$ acts as a *load* on the output of the voltage divider, meaning it causes a current $i_\text L$ to flow. The presence of $\text R_\text L$ means $\text{R1}$ and $\text{R2}$ are no longer strictly in series. We don't want $i_\text L$ to be very large (much smaller than $i_1$ or $i_2$), so let's make $\text{R}\_{\text L}$ fairly large. Let $\text R_\text L$ be ten times bigger than $\text{R2}$,

$\text{R}_{\text L} = 10\,\text{R2}$

With this load resistor in place, we will take a look at what happens to the output voltage. 

$\text{R2}$ and $\text{R}\_{\text L}$ are in parallel with each other. Combine the two parallel resistors using the parallel resistor formula,  

$\text{R2} \,\|\|\, \text{R}\_{\text L} = \dfrac{\text{R2}\cdot\text{R}_{\text L}}{\text{R2}+\text{R}\_{\text L}} = \dfrac{\text{R2}\cdot10\,\text{R2}}{\text{R2}+10\,\text{R2}} = \dfrac{10}{11}\,\text{R2} = 0.91\,\text{R2}$

<details>
<summary>Parallel resistor formula</summary>
<p>Two parallel resistors, $\text R_x$ and $\text R_y$ (parallel resistors share the same two nodes) can be combined to get an equivalent resistor like this, </p>
...
<details>
<summary>What do the vertical bars mean?</summary>
<p>The pair of vertical bars $||$ is just a shorthand notation meaning "in parallel with."</p>

<p>$\text{R2} \,||\, \text{R}_{\text L}$ means "$\text{R2}$ in parallel with $\text{R}_{\text L}$."</p>
</details>

This is our loaded voltage divider circuit, redrawn to show the equivalent  resistance of $\text R2$ in parallel with $\text R_\text L$,

![](https://ka-perseus-images.s3.amazonaws.com/63b5ae1b7a5a3f4723ba6ceb9935e3175cb22845.svg){: .centered }

The $10{\times}$ load resistor has the effect of reducing the resistance at the bottom of the voltage divider by roughly $9\%$. What is the impact of this additional load on the divider's output voltage? Without the load, the expected output is $0.5\,v_{in}$. Now we figure out the output voltage in the presence of a load resistor. 
...
The actual output of the voltage divider is low by $4\%$ compared to the designed voltage. Notice how the $4\%$ voltage error is significantly less than the $9\%$ resistance change. Does a few $\%$ error matter? That is for you alone to decide. It depends on how accurate the voltage divider needs to be for your application. 
...
That means $\text R2$ is $9$ times bigger than $\text R1$. 
...
We replace $\text R2$ with $9\,\text R1$,
...
The actual output voltage is $89\%$ of $v_{in}$ instead of $90\%$.

The actual output voltage divided by the expected output is,

$\dfrac{0.89}{0.90} = 0.99$

So the actual voltage is lower than the expected by only $1\%$.
...
Let $v_{out} = 10\%$ of $v_{in}$. The expected output is $0.10 \,v_{in}$. 
...
$\text R1$ is $9$ times bigger than $\text R2$. 

Now we load the circuit with $\text R_\text L$ and evaluate the change in output voltage. The expression for the loaded voltage divider is still, 
...
$\dfrac{0.091}{0.10} = 0.91$

So the actual voltage differs by $9\%$ from the expected. This is nearly twice the error compared to the mid-range divider.
...
* Near the top of its range, the error goes down substantially, to around $1\%$. 
...
Real-world resistors always have a $\pm$ tolerance on their value. If the accuracy of the voltage divider is critical to your application, use resistors with tight tolerances, and check for acceptable performance by analyzing the voltage divider at the anticipated tolerance extremes.
